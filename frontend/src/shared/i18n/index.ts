  ...
  "zh-CN": {
    translation: {
      ...
      settings: {
        ...
        globalProxy: { title: "全局出口代理", enabled: "启用全局代理", enabledHelp: "启用后所有 Build、Web、Console 及 Provider 媒体出站连接都强制使用此代理，并覆盖按账号配置的出口节点；关闭时恢复原有直连或节点代理。", scheme: "代理协议", host: "代理主机", port: "代理端口", username: "用户名（可选）", password: "密码（可选）", keepPassword: "留空保持当前已保存密码；清空用户名可移除认证。" },
...
Object.assign(resources.en.translation.settings as unknown as Record<string, unknown>, {
  globalProxy: {
    ...
    enabledHelp: "Route all Build, Web, Console, and Provider media outbound connections through this proxy and override account-level egress nodes. Disabling it restores existing direct or node routing.",
